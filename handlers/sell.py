...
async def notify_admins(message: Message, listing_id: int, data: dict, language: str):
    for admin_id in config.ADMIN_IDS:
        try:
            escaped_data = {
                k: Button.escape_markdown(str(v)) 
            ...
            await message.bot.send_message(
                chat_id=admin_id,
                text=get_string('new_listing', language).format(
                    username=username,
                    **escaped_data
                ),
            ...
            if data.get('image_file_id'):
                await message.bot.send_photo(
                    ...
                    caption=Button.escape_markdown(f"Image for listing #{listing_id}")
                )
...
async def sell_code_start(message: Message, state: FSMContext, **kwargs):
    ...
    await message.answer(
        text=get_string('sell_code_start', language),
        parse_mode="Markdown"
    )
...
async def process_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=get_string('enter_project_link', language),
        parse_mode="Markdown"
    )
...
async def process_link(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=get_string('enter_technologies', language),
        parse_mode="Markdown"
    )
...
async def process_technologies(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=get_string('enter_price', language),
        parse_mode="Markdown"
    )
...
async def process_price(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            text=get_string('enter_description', language),
            parse_mode="Markdown"
        )
...
async def process_description(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=get_string('enter_image', language),
...
async def process_image(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=get_string('confirm_listing', language).format(**state_data),
...
async def confirm_listing(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=get_string('listing_submitted', language),
